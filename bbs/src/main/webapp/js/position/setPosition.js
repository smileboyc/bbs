var nowPro = '湖北省';
		 ...
		 function setup(Select) {
		    ...
		    for (i = 0; i < Array_province.length; i++) {
		     document.getElementById(Select).options.add(new Option(Array_province[i], Array_province[i]));
		    }		 
		}
		//填充市级城市（参数：下拉列表的ID）
		 function select4change(Select4,Select5) {			
		    var nub = 1+document.getElementById(Select4).selectedIndex;
		    var select5 = document.getElementById(Select5);
		    var selecta = pro_City[nub-1].split(',');//要慢一拍,为何?
		    if (selecta.length > 0) {
		    document.getElementById(Select5).options.length = 0;
		    for (i = 1; i < selecta.length; i++)
		    document.getElementById(Select5).options.add(new Option(selecta[i], selecta[i]));
		    }
		 }